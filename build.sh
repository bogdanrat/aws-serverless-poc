...
sam deploy --template-file ./template-output.dev.yaml --stack-name ${SAM_STACK_NAME} --capabilities CAPABILITY_IAM CAPABILITY_AUTO_EXPAND --region ${DEPLOYMENT_REGION} --s3-bucket ${DEPLOYMENT_BUCKET_NAME} || exit
